 ...
 def main() -> None:
     ...
     def predict_proba(a: DrawEntry, b: DrawEntry) -> float:
         ...
         row = build_feature_row(
             ...
             p2_id=pid_b,
             surface=surface,
             tourney_level="G",  # MVP default; override later using real metadata
             round_name="R128",
             best_of=5 if args.tournament.lower() in {"us open", "australian open", "roland garros", "wimbledon"} else 3,
             p1_rank=rank_a,
             p2_rank=rank_b,
             p1_points=pts_a,
             p2_points=pts_b,
     ...
     report_path = Path(args.report) if args.report else (paths.reports / f"{args.tournament.lower().replace(' ', '_')}_{args.year}_prediction.md")
     write_prediction_report_md(
         ...
         notes=[
             ...
             "This MVP uses a simple power-of-two ordered bracket (slot1 vs slot2, ...).",
         ],
     )
